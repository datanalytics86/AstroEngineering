...
> 2026-06-23 (v2) — Filtro por tópico en tránsitos. `lib/topics.ts` taxonomía de 7 tópicos (dinero, amor, carrera, hogar, mente, crecimiento, energía) + mapeo bidireccional ES/EN de `life_areas` → TopicId. `lib/topic-summary.ts` selección top-12, outlook meter y párrafo bibliográfico bilingüe (Hand/Sasportas/Arroyo). `components/TransitTopicTimeline.tsx` Gantt horizontal con click-to-expand por tránsito (detailed + advice), línea "hoy", scroll móvil. `app/transitos/[id]/page.tsx` chip row ("Todos" + 7 tópicos) + `TopicPanel` (aplica a año actual y futuros; "Todos" preserva el comportamiento existente). i18n completo ES/EN. Nuevos archivos: `lib/topics.ts`, `lib/topic-summary.ts`, `components/TransitTopicTimeline.tsx`.
